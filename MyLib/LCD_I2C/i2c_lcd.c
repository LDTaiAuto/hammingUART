...
/**
 * @brief  Sends an integer as hexadecimal string to the LCD.
 * @param  lcd: Pointer to the LCD handle
 * @param  num: Integer number to display in hexadecimal
 * @retval None
 */
void lcd_put_hex(I2C_LCD_HandleTypeDef *lcd, uint8_t num)
{
    char buffer[11];  // Enough for "0xFFFFFFFF" + null terminator
    sprintf(buffer, "0x%X", num);  // Convert number to HEX string
    lcd_puts(lcd, buffer);         // Send the string to LCD
}
/**
 * @brief  Sends an integer as binary string to the LCD.
 * @param  lcd: Pointer to the LCD handle
 * @param  num: Integer number to display in binary
 * @retval None
 */
void lcd_put_bin(I2C_LCD_HandleTypeDef *lcd, uint8_t num)
{
    char buffer[35];  // 32 bits + "0b" + null terminator
    buffer[0] = '0';
    buffer[1] = 'b';
    
    for (int i = 31; i >= 0; i--) {
        buffer[33 - i] = (num & (1 << i)) ? '1' : '0';
    }
    buffer[34] = '\0';  // Null terminator
    
    lcd_puts(lcd, buffer);  // Display the binary string
}
